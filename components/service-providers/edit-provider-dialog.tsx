...
import { ServiceProvider, ProviderCategory } from '@/lib/hooks/use-service-providers'

interface EditProviderDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  provider: ServiceProvider | null
  onSubmit: (id: string, updates: Partial<ServiceProvider>) => Promise<void>
}

// Categories matching the add dialog
...
export function EditProviderDialog({ open, onOpenChange, provider, onSubmit }: EditProviderDialogProps) {
  ...
  useEffect(() => {
    if (provider) {
      setFormData({
        ...
        monthly_amount: provider.monthly_amount?.toString() || '',
        billing_day: provider.billing_day?.toString() || '1',
        phone: provider.phone || '',
        website: provider.website || '',
        auto_pay_enabled: provider.auto_pay_enabled || false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.provider_name || !provider) {
      return
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">Edit Provider</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 mt-4">
          ...
          {/* Monthly Amount & Billing Day - Side by side */}
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="edit_auto_pay" className="text-slate-300 cursor-pointer">
              Auto-pay enabled
            </Label>
          </div>
